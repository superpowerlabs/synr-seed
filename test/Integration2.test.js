...
describe("#Integration2 test", function () {
  ...
  let MainTesseract, mainTesseract;
  let MainPool, mainPool;
  let SeedToken, seed;
  let SynCityPasses, pass;
  let SideTesseract, sideTesseract;
  ...
  before(async function () {
    ...
    MainTesseract = await ethers.getContractFactory("MainTesseractMock");
    SideTesseract = await ethers.getContractFactory("SideTesseractMock");
  ...
  async function initAndDeploy() {
    ...
    mainTesseract = await MainTesseract.deploy(mainPool.address);
    ...
    await mainTesseract.wormholeInit(2, wormhole.address);
    await wormhole.deployed();

    await mainTesseract.wormholeRegisterContract(4, bytes32Address(sideTesseract.address));
    await mainPool.initPool(7, 4000);

    await sideTesseract.wormholeInit(4, wormhole.address);
    await sideTesseract.wormholeRegisterContract(2, bytes32Address(mainTesseract.address));
  }
  ...
  it("should manage the entire flow", async function () {
    ...
    // stake SYNR in the MainTesseract
    ...
    expect(
      await mainTesseract.connect(fundOwner).wormholeTransfer(
        payload,
        4, // BSC
    ...
    expect(
      mainTesseract.connect(user2).wormholeTransfer(
        payload3,
        4, // BSC
        bytes32Address(fundOwner.address),
        1
      )
    ).revertedWith("MainTesseract: only the sender can receive on other chain");

    expect(
      await mainTesseract.connect(user2).wormholeTransfer(
        payload3,
        4, // BSC
    ...
    expect(await sideTesseract.mockWormholeCompleteTransfer(fundOwner.address, finalPayload))
    ...
    expect(await sideTesseract.mockWormholeCompleteTransfer(fundOwner.address, finalPayload2))
      .emit(sideTesseract, "DepositSaved")
      .withArgs(fundOwner.address, 1);
    ...
    expect(await sideTesseract.mockWormholeCompleteTransfer(user2.address, finalPayload3))
      .emit(sideTesseract, "DepositSaved")
      .withArgs(user2.address, 0);
    ...
    expect(await sideTesseract.mockWormholeCompleteTransfer(fundOwner.address, finalPayload4))
      .emit(sideTesseract, "DepositSaved")
      .withArgs(fundOwner.address, 2);
    ...
    expect(await sideTesseract.connect(fundOwner).wormholeTransfer(seedPayload, 2, bytes32Address(fundOwner.address), 1))
  ...
  it("should verify early unstake", async function () {
    ...
    await mainTesseract.connect(user1).wormholeTransfer(
      payload,
      4, // BSC
    ...
    await sideTesseract.connect(user1).mockWormholeCompleteTransfer(user1.address, finalPayload);
    ...
    // unstake
    await sideTesseract.connect(user1).wormholeTransfer(seedPayload, 2, bytes32Address(user1.address), 1);
    ...
    await mainTesseract.mockWormholeCompleteTransfer(user1.address, seedPayload);
  ...
  it("should stake pass for boost and increase boostWeight", async function () {
    ...
    await sideTesseract.connect(fundOwner).mockWormholeCompleteTransfer(fundOwner.address, finalPayloadSynr);
    ...
    const finalPayload = await fromDepositToTransferPayload(deposit);
    await sideTesseract.connect(fundOwner).mockWormholeCompleteTransfer(fundOwner.address, finalPayload);
    ...
    await mainTesseract.mockWormholeCompleteTransfer(fundOwner.address, seedPayload);
